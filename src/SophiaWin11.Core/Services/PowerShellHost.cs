...
public sealed class PowerShellHost : IPowerShellHost
{
    ...
    public async Task<IReadOnlyList<string>> InvokeAndCaptureAsync(string script, CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(script))
        {
            return [];
        ...
        var output = results.Select(result => result?.ToString() ?? string.Empty).ToList();
        _logger.LogInformation("PowerShell script executed with {Count} output records.", output.Count);
